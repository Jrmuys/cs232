...
class CPU(threading.Thread):
    ...
    def set_batch_array(self, mem_val):
        ''' Set memory location for batch array '''
        self._batch_array = mem_val

    def set_batch(self, boolean):
        ''' Set cpu into batch mode '''
        self._batch = boolean
    ...
    def run(self):
        ...
        if (self._batch):
            array_start = self._batch_array
            if self._debug:
                print("Starting batchmode...")
            self.set_pc(self._ram.__getitem__(array_start))
            while (self._ram.__getitem__(array_start) != 0):
                if self._debug:
                    print("Running program at PC: " + str(self._registers['pc']))
                self.clear_registers()
                self._run_program()
                if self._debug:
                    print("Ran program successfully")
                array_start = array_start + 1
                self.set_pc(self._ram[array_start])
                if self._debug:
                    print("Value at next pc memory [" + str(self._registers['pc']) + "] location:" + str(self._ram.__getitem__(self._registers['pc'])))
