...
class CalOS:
    ...
    def timer_isr(self):
        ...
        # Set the resitsers in the current proccess to the cpu's current registers
        self.current_proc.set_registers(self._cpu.get_registers())
        
        if self._debug:
            print("End of quantum, ready queue is size", len(self._ready_q))

        # Check if there is stuff in the ready_q, if so, context switch
    ...
    def context_switch(self):
        ...
        if self._debug:
            print("context_switch from" + str(self.current_proc.get_name() + " to " + str(new_PCB.get_name())))
        self.current_proc.set_registers(self._cpu.get_registers())
    ...
    def run(self):
        ...
        while(len(self._ready_q) > 0):
            ...
            if self._debug:
                print("Current proc: " + str(self.current_proc))
            self.reset_timer()
